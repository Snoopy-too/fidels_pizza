...
const RegisterPage: React.FC = () => {
    ...
    const handleCodeCheck = (e: React.FormEvent) => {
        ...
        if (enteredCode === correctAccessCode) {
            ...
            setError(t('registerPage.invalidCodeError'));
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (newUser) {
            showAlert(t('registerPage.successTitle'), t('registerPage.successMessage'), () => {
                navigate('/menu');
            });
        } else {
            setError(t('registerPage.emailExistsError'));
        }
    };

    return (
        <div className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-lg">
            <h2 className="text-3xl font-bold text-center text-stone-800 mb-6">{t('registerPage.title')}</h2>
            
            {!isCodeVerified ? (
                <form onSubmit={handleCodeCheck} className="space-y-4">
                    <p className="text-center text-stone-600">{t('registerPage.accessCodePrompt')}</p>
                    <div>
                        <label htmlFor="accessCode" className="block text-sm font-medium text-stone-700">{t('registerPage.accessCodeLabel')}</label>
                        <input
                            type="text"
                            id="accessCode"
                            value={enteredCode}
                    ...
                    <button type="submit" className="w-full bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        {t('registerPage.verifyCodeBtn')}
                    </button>
                </form>
            ) : (
                <form onSubmit={handleSubmit} className="space-y-4 animate-fade-in">
                    <p className="text-center text-green-600 font-semibold">{t('registerPage.codeVerifiedSuccess')}</p>
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-stone-700">{t('registerPage.fullNameLabel')}</label>
                    ...
                    <div>
                        <label htmlFor="email" className="block text-sm font-medium text-stone-700">{t('registerPage.emailLabel')}</label>
                    ...
                    <div>
                        <label htmlFor="password" className="block text-sm font-medium text-stone-700">{t('registerPage.passwordLabel')}</label>
                    ...
                    {error && <p className="text-red-500 text-sm">{error}</p>}
                    <p className="text-xs text-stone-500">{t('registerPage.simulationNotice')}</p>
